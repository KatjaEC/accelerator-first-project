  ...
  paginationButtons.forEach((button) => {
    for (let i = 0; i < visibleBullets.length; i++) {
      ...
      if (document.body.clientWidth >= tabletWidth) {
        ...
        } else if ([...visibleBullets].indexOf(visibleBullets[0]) === activeBulletIndex && [...visibleBullets].indexOf(visibleBullets[1]) === activeBulletIndex && activeBullet !== paginationButtons[0]) {
          button.style.left = `${currentTransitionValue + 52}px`;
      ...
      console.log([...visibleBullets].indexOf(visibleBullets[1]));
    }
  });
  console.log(visibleBullets[3]);
});
